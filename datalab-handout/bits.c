...
int maximum = (1<<31)^(~0);
int minimum = ~maximum;
int c = (add_result>>31)&1;
int overflow = ~((!(a^b))&(a^c)) + 1;
int d = ~c + 1;
  return (overflow|add_result)&((~overflow)|(d&maximum)|((~d)&minimum));
}
...
unsigned float_half(unsigned uf) {
  ...
  int ufMask = 0x807FFFFF;
  int exponent = uf&expMask;
  int beforeRounding = 0;
  if(exponent == expMask){
    return uf;
  }
  if(!(exponent>>24)){
    beforeRounding = ((uf>>1) + 0x40000000)&ufMask;
    if((uf&0x3) == 0x3){
      return (beforeRounding + 1);
    }
